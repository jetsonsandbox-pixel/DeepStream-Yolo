#!/usr/bin/env python3
"""
Daylight Camera with Inference (NO PREPROCESSING/TILING)
Direct camera → mux → infer → sink
Tests inference pipeline without custom preprocessing
"""
...
class DaylightInferenceTest:
    def __init__(self):
        Gst.init(None)
        self.pipeline = Gst.Pipeline.new("daylight-infer")
    ...
    def build_pipeline(self):
        """Build daylight camera → inference pipeline (NO tiling)"""
        # Source: CSI camera
        ...
        # Inference using tiling config with batch-size=1 (no actual tiling, just single batch)
        infer = Gst.ElementFactory.make("nvinfer", "infer")
        # Use tiling config since b1 fp32 engine doesn't exist - we'll override batch-size
        ...
        # Link pipeline with preprocessing
        mux.link(preprocess)
