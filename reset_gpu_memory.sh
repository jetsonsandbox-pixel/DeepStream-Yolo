#!/bin/bash

# GPU Memory Reset Script for DeepStream Dual-Camera Pipeline
# Use this to clear GPU memory between pipeline runs if needed

echo "=========================================="
echo "DeepStream GPU Memory Reset Tool"
echo "=========================================="
echo ""

echo "📊 Current GPU Memory Status:"
if command -v tegrastats &> /dev/null; then
    echo "   Using tegrastats for Jetson:"
    timeout 2s tegrastats | head -1
else
    echo "⚠️  tegrastats not available"
fi
echo ""

echo "🔍 Checking for running DeepStream pipeline processes..."
PIPELINE_PIDS=$(pgrep -f "dual_cam_pipeline.py")
if [ -n "$PIPELINE_PIDS" ]; then
    echo "⚠️  Found running pipeline processes:"
    ps aux | grep dual_cam_pipeline.py | grep -v grep
    echo ""
    read -p "Kill these processes? (y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo "🔫 Killing pipeline processes..."
        pkill -9 -f dual_cam_pipeline.py
        sleep 1
        echo "✅ Processes killed"
    fi
else
    echo "✅ No pipeline processes running"
fi
echo ""

echo "🔍 Checking for stuck GStreamer/DeepStream processes..."
GSTREAMER_PIDS=$(pgrep -f "gst-launch\|nvstreammux\|nvinfer")
if [ -n "$GSTREAMER_PIDS" ]; then
    echo "⚠️  Found GStreamer/DeepStream processes:"
    ps aux | grep -E "gst-launch|nvstreammux|nvinfer" | grep -v grep
    echo ""
    read -p "Kill these processes? (y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo "🔫 Killing GStreamer/DeepStream processes..."
        pkill -9 -f "gst-launch"
        pkill -9 -f "nvstreammux"
        pkill -9 -f "nvinfer"
        sleep 1
        echo "✅ Processes killed"
    fi
else
    echo "✅ No stuck GStreamer/DeepStream processes"
fi
echo ""

echo "🧹 Clearing Python cache and temporary files..."
find /tmp -name "*.pyc" -user $USER -delete 2>/dev/null
find /tmp -name "__pycache__" -user $USER -type d -exec rm -rf {} + 2>/dev/null
echo "✅ Python cache cleared"
echo ""

echo "🔄 Forcing Python garbage collection and CUDA cache clear..."
python3 << 'EOF'
import gc
import sys

# Force garbage collection
gc.collect()
print("✅ Python GC completed")

# Clear CUDA cache if PyTorch available
try:
    import torch
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
        print("✅ PyTorch CUDA cache cleared")
    else:
        print("ℹ️  CUDA not available via PyTorch")
except ImportError:
    print("ℹ️  PyTorch not available")

# Clear TensorRT/CUDA resources
try:
    import ctypes
    libcudart = ctypes.CDLL('libcudart.so')
    libcudart.cudaDeviceReset()
    print("✅ CUDA device reset")
except Exception as e:
    print(f"ℹ️  CUDA device reset not available: {e}")
EOF
echo ""

echo "📊 Checking nvargus-daemon (CSI camera daemon)..."
if pgrep -f nvargus-daemon > /dev/null; then
    NVARGUS_MEM=$(ps aux | grep nvargus-daemon | grep -v grep | awk '{print $6}')
    echo "✅ nvargus-daemon is running (Memory: ${NVARGUS_MEM}KB)"
    echo ""
    read -p "Restart nvargus-daemon to free CSI camera resources? (y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        echo "🔄 Restarting nvargus-daemon..."
        sudo systemctl restart nvargus-daemon
        sleep 2
        if pgrep -f nvargus-daemon > /dev/null; then
            echo "✅ nvargus-daemon restarted successfully"
        else
            echo "❌ nvargus-daemon failed to restart"
        fi
    fi
else
    echo "⚠️  nvargus-daemon is NOT running!"
    read -p "Start nvargus-daemon for CSI camera? (y/n): " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        sudo systemctl start nvargus-daemon
        sleep 2
        if pgrep -f nvargus-daemon > /dev/null; then
            echo "✅ nvargus-daemon started successfully"
        else
            echo "❌ nvargus-daemon failed to start"
        fi
    fi
fi
echo ""

echo "🔍 Checking camera devices..."
echo "   CSI Camera (/dev/video0):"
if [ -e /dev/video0 ]; then
    echo "   ✅ Available"
    if command -v lsof &> /dev/null; then
        LOCKS=$(sudo lsof /dev/video0 2>/dev/null)
        if [ -z "$LOCKS" ]; then
            echo "      ✅ Not locked by any process"
        else
            echo "      ⚠️  Locked by:"
            echo "$LOCKS" | sed 's/^/      /'
        fi
    fi
else
    echo "   ❌ Not found"
fi

echo "   Thermal USB Camera (/dev/video1):"
if [ -e /dev/video1 ]; then
    echo "   ✅ Available"
    if command -v v4l2-ctl &> /dev/null; then
        V4L2_INFO=$(v4l2-ctl --device=/dev/video1 --info 2>/dev/null | grep "Card type" | cut -d: -f2)
        echo "      Device:$V4L2_INFO"
    fi
    if command -v lsof &> /dev/null; then
        LOCKS=$(sudo lsof /dev/video1 2>/dev/null)
        if [ -z "$LOCKS" ]; then
            echo "      ✅ Not locked by any process"
        else
            echo "      ⚠️  Locked by:"
            echo "$LOCKS" | sed 's/^/      /'
        fi
    fi
else
    echo "   ❌ Not found"
fi
echo ""

echo "⏳ Waiting 3 seconds for system to stabilize..."
sleep 3
echo ""

echo "=========================================="
echo "📊 Final System Status"
echo "=========================================="
echo ""

echo "1. GPU Memory:"
if command -v tegrastats &> /dev/null; then
    echo "   Using tegrastats:"
    timeout 2s tegrastats | head -1
else
    echo "   ℹ️  tegrastats not available"
fi
echo ""

echo "2. Camera Devices:"
if [ -e /dev/video0 ]; then
    echo "   ✅ CSI /dev/video0 available"
    if command -v lsof &> /dev/null; then
        LOCKS=$(sudo lsof /dev/video0 2>/dev/null)
        if [ -z "$LOCKS" ]; then
            echo "      ✅ Not locked"
        else
            echo "      ⚠️  Locked by process"
        fi
    fi
else
    echo "   ❌ CSI /dev/video0 not found"
fi

if [ -e /dev/video1 ]; then
    echo "   ✅ USB /dev/video1 available"
    if command -v lsof &> /dev/null; then
        LOCKS=$(sudo lsof /dev/video1 2>/dev/null)
        if [ -z "$LOCKS" ]; then
            echo "      ✅ Not locked"
        else
            echo "      ⚠️  Locked by process"
        fi
    fi
else
    echo "   ❌ USB /dev/video1 not found"
fi
echo ""

echo "3. nvargus-daemon:"
if pgrep -f nvargus-daemon > /dev/null; then
    echo "   ✅ Running"
    ps aux | grep nvargus-daemon | grep -v grep | awk '{print "   Memory:", $6/1024, "MB"}'
else
    echo "   ❌ Not running"
fi
echo ""

echo "4. DeepStream/Python Processes:"
if pgrep -f "dual_cam_pipeline.py\|nvstreammux\|nvinfer" > /dev/null; then
    echo "   ⚠️  Processes still running:"
    ps aux | grep -E "dual_cam_pipeline.py|nvstreammux|nvinfer" | grep -v grep
else
    echo "   ✅ No processes running"
fi
echo ""

echo "=========================================="
echo "✅ GPU Memory Reset Complete!"
echo "=========================================="
echo ""
echo "💡 You can now run the dual-camera pipeline:"
echo "   cd /home/jet-nx8/DeepStream-Yolo"
echo "   python3 dual_cam_pipeline.py"
echo ""
echo "📋 TensorRT Engine Files:"
if [ -f "/home/jet-nx8/DeepStream-Yolo/model_b8_gpu0_fp16.engine" ]; then
    ENGINE_SIZE=$(ls -lh /home/jet-nx8/DeepStream-Yolo/model_b8_gpu0_fp16.engine | awk '{print $5}')
    echo "   ✅ Daylight: model_b8_gpu0_fp16.engine ($ENGINE_SIZE)"
else
    echo "   ⚠️  Daylight engine not found"
fi
if [ -f "/home/jet-nx8/DeepStream-Yolo/model_thermal_b1_gpu0_fp16.engine" ]; then
    ENGINE_SIZE=$(ls -lh /home/jet-nx8/DeepStream-Yolo/model_thermal_b1_gpu0_fp16.engine | awk '{print $5}')
    echo "   ✅ Thermal: model_thermal_b1_gpu0_fp16.engine ($ENGINE_SIZE)"
else
    echo "   ⚠️  Thermal engine not found"
fi
echo ""
